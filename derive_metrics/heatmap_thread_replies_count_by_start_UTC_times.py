...
with conn:
    ...
    for i,(start,end) in enumerate(header.BINS_IDS_RANGES):
        ...
        for (thread_id,epoch) in threads:
            cur.execute(
                ''' SELECT Count()
                    FROM Posts
                    WHERE THREAD_ID=?
            ...
            heatmap[i][hour] += replies_count * .5
            heatmap[i][(hour+1) % 24] += replies_count * .25
            heatmap[i][(hour-1) % 24] += replies_count * .25
...
ax.set_title("Number of Replies in Thread by Thread Creation UTC Hour by Bin\nSmoothed by 1 Hour", fontweight='bold')
...
plt.savefig('graphs/heatmap_thread_replies_count_by_start_UTC_times.png',dpi=header.IMG_DPI)
