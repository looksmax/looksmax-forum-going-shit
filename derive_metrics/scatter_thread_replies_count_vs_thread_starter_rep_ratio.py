...
with conn:
    ...
    for i,(start,end) in enumerate(header.BINS_IDS_RANGES):
        ...
        cur.execute(
        f'''
            SELECT (CAST(REP_SCORE AS REAL) / CAST(POSTS_COUNT AS REAL)),
                (
        ...
        for (rep_ratio,n_replies) in tqdm.tqdm(list(cur.fetchall())):
            x.append(n_replies)
            y.append(rep_ratio)
...
plt.savefig('graphs/scatter_thread_replies_count_vs_thread_starter_rep_ratio.png',dpi=header.IMG_DPI)
